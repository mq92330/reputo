...
export default function DashboardPage() {
  ...
  async function handleSend(e) {
    ...
    const res = await fetch('/api/send-sms', {
      ...
      body: JSON.stringify({ phone, patient_name: name }),
    });
    const data = await res.json();
    setResult(data);
    setSending(false);
    if (data.ok) { setPhone(''); setName(''); load(); }
  }
        ...
        <form onSubmit={handleSend} className={styles.sendForm}>
          <div className={styles.formRow}>
            <div className={styles.field}>
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Marie" />
            </div>
            <div className={styles.field}>
              <label>Numéro de téléphone</label>
              <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="06 12 34 56 78" required />
            </div>
          </div>
